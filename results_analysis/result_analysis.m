...
%% AP 
% experimental data
ap_exp_wt = table2array(readtable('15210016.xlsx'));
ap_exp_ko = table2array(readtable('15211017.xlsx'));

t1 = ap_exp_wt(:,1);
ap_exp_wt_avg = mean(ap_exp_wt(:,2:6),2);

t2 = ap_exp_ko(:,1);
ap_exp_ko_avg = mean(ap_exp_ko(:,2:6),2);

cut_time = 250;
[~,t1_idx] = min(abs(t1-cut_time));
[~,t2_idx] = min(abs(t2-cut_time));

t1 = t1(1:t1_idx);
ap_exp_wt_avg = ap_exp_wt_avg(1:t1_idx);

t2 = t2(1:t2_idx);
ap_exp_ko_avg = ap_exp_ko_avg(1:t2_idx);
...
subplot(1,2,1)
plot(t1,ap_exp_wt_avg, 'Color','blue','LineWidth',1.5)
hold on
plot(t2,ap_exp_ko_avg, 'Color','red','LineWidth',1.5)
yline(0,'--')
hold off
axis tight
box off
legend('WT','Mgat1KO')
xlabel('Time (ms)')
ylabel('Experimental AP (mV)')
set(gca,'FontName','Arial','FontSize',12,'FontWeight','bold')

subplot(1,2,2)
plot(ap_wt(:,1),ap_wt(:,2), 'Color','blue','LineWidth',1.5)
hold on
plot(ap_ko(:,1),ap_ko(:,2), 'Color','red','LineWidth',1.5)
yline(0,'--')
hold off
axis tight
box off
