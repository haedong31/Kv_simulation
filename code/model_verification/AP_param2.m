...
function [t, STATES, ALGEBRAIC, CONSTANTS] = AP_param2(X, Ek, sim_time)
    % This is the "main function".  In Matlab, things work best if you rename this function to match the filename.
   [t, STATES, ALGEBRAIC, CONSTANTS] = solveModel(X, Ek, sim_time);
end

function [t, STATES, ALGEBRAIC, CONSTANTS] = solveModel(X, Ek, sim_time)
    ...
    [t, STATES] = ode15s(@(t, STATES)computeRates(X, Ek, t, STATES, CONSTANTS), tspan, INIT_STATES, options);

    % Compute algebraic variables
    [RATES, ALGEBRAIC] = computeRates(X, Ek, t, STATES, CONSTANTS);
    ALGEBRAIC = computeAlgebraic(X, Ek, ALGEBRAIC, CONSTANTS, STATES, t);
end

function [RATES, ALGEBRAIC] = computeRates(X, Ek, t, STATES, CONSTANTS)
    ...
    % Ito
    % A71; alpha_a 
    ALGEBRAIC(:,72) =  X(1).*exp( X(2).*(STATES(:,1)+X(3)));
    % A72; beta_a 
    ALGEBRAIC(:,73) =  X(4).*exp(  X(5).*(STATES(:,1)+X(6)));
    % A73; alpha_i
    ALGEBRAIC(:,74) = ( 0.000152000.*exp( - (STATES(:,1)+X(3))./7.0))./( 0.00670830.*exp( - (STATES(:,1)+33.5)./7.0)+1.00000);
    % A74; beta_i
    ALGEBRAIC(:,75) = ( 0.000950000.*exp((STATES(:,1)+X(4))./X(5)))./( 0.0513350.*exp((STATES(:,1)+X(4))./X(5))+1.00000);
    % A69; ato_f
    RATES(:,42) =  ALGEBRAIC(:,72).*(1.00000 - STATES(:,42)) -  ALGEBRAIC(:,73).*STATES(:,42);
    % A70; ito_f
    RATES(:,43) =  ALGEBRAIC(:,74).*(1.00000 - STATES(:,43)) -  ALGEBRAIC(:,75).*STATES(:,43);
    % A67; I_Kto,f
    ALGEBRAIC(:,76) =  X(6).*power(STATES(:,42), 3.00000).*STATES(:,43).*(STATES(:,1) - Ek);

    % IKslow1
    % A78; a_ss
    ALGEBRAIC(:,77) = 1.00000./(1.00000+exp( - (STATES(:,1)+X(7))./X(8)));
    % A79; i_ss
    ALGEBRAIC(:,78) = 1.00000./(1.00000+exp((STATES(:,1)+X(9))./X(10)));
    % A90; tau_aur
    ALGEBRAIC(:,79) =  0.493000.*exp(  - 0.0629000.*STATES(:,1)) + 2.058;
    % A91; tau_iur
    ALGEBRAIC(:,80) = X(11) - 170.000./(1.00000+exp((STATES(:,1)+45.2)./5.7));
    % A88; aur
    RATES(:,44) = (ALGEBRAIC(:,77) - STATES(:,44))./ALGEBRAIC(:,79);
    % A89; iur
    RATES(:,45) = (ALGEBRAIC(:,78) - STATES(:,45))./ALGEBRAIC(:,80);
    % A87; I_Kur
    ALGEBRAIC(:,81) =  X(12).*STATES(:,44).*STATES(:,45).*(STATES(:,1) - Ek);
    
    % IKslow2
    % A78; a_ss
    ALGEBRAIC(:,82) = 1.00000./(1.00000+exp( - (STATES(:,1)+X(13))./X(14)));
    % A79; i_ss
    ALGEBRAIC(:,83) = 1.00000./(1.00000+exp((STATES(:,1)+X(15))./X(16)));
    % A90; tau_aur
    ALGEBRAIC(:,84) =  0.493000.*exp(  - 0.0629000.*STATES(:,1)) + 2.058;
    % A91; tau_iur
    ALGEBRAIC(:,85) = X(17) - 170.000./(1.00000+exp((STATES(:,1)+45.2)./5.7));
    % A88; aur
    RATES(:,46) = (ALGEBRAIC(:,82) - STATES(:,46))./ALGEBRAIC(:,84);
    % A89; iur
    RATES(:,47) = (ALGEBRAIC(:,83) - STATES(:,47))./ALGEBRAIC(:,85);
    % A87; I_kUR
    ALGEBRAIC(:,86) =  X(18).*STATES(:,46).*STATES(:,47).*(STATES(:,1) - Ek);
    
    % A1; V
    RATES(:,1) =  - (ALGEBRAIC(:,47)+ALGEBRAIC(:,49)+ALGEBRAIC(:,51)+ALGEBRAIC(:,55)+ALGEBRAIC(:,58)+ALGEBRAIC(:,59)+ALGEBRAIC(:,69)+ALGEBRAIC(:,62)+ALGEBRAIC(:,63)+ALGEBRAIC(:,64)+ALGEBRAIC(:,66)+ALGEBRAIC(:,67)+ALGEBRAIC(:,71)+ALGEBRAIC(:,1)+ALGEBRAIC(:,76)+ALGEBRAIC(:,81)+ALGEBRAIC(:,86));
    RATES = RATES';
end

function ALGEBRAIC = computeAlgebraic(X, Ek, ALGEBRAIC, CONSTANTS, STATES, t)
    ALGEBRAIC(:,2) = 1.00000 - (STATES(:,11)+STATES(:,9)+STATES(:,10));
    ALGEBRAIC(:,5) =  0.180640.*exp( 0.0357700.*(STATES(:,1)+30.0000));
    ALGEBRAIC(:,15) =  0.395600.*exp(  - 0.0623700.*(STATES(:,1)+30.0000));
    ALGEBRAIC(:,6) = ( 0.000152000.*exp( - (STATES(:,1)+13.5000)./7.00000))./( 0.00670830.*exp( - (STATES(:,1)+33.5000)./7.00000)+1.00000);
...
function [STATES, CONSTANTS] = initConsts()
    ...
    STATES(:,42) = 0.265563e-2;  % ato_f; Gating variable for transient outward K+ current
    STATES(:,43) = 0.999977;  % ito_f; Gating variable for transient outward K+ current
    STATES(:,44) = 0.417069e-3;  % aur; Gating variable for ultrarapidly activating delayed-rectifier K+ current
    STATES(:,45) = 0.998543;  % iur; Gating variable for ultrarapidly activating delayed-rectifier K+ current0
    STATES(:,46) = 0.417069e-3;  % aur; Gating variable for ultrarapidly activating delayed-rectifier K+ current
    STATES(:,47) = 0.998543;  % iur; Gating variable for ultrarapidly activating delayed-rectifier K+ current
